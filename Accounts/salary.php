        ...
        <div data-toggle="tooltip" data-placement="top" data-trigger="hover">
            <form id="monthYearForm" class="d-flex align-items-center w-100">
                <div id="customMonthYearPicker" class="custom-month-year-picker d-flex align-items-center w-100">
                    <select id="monthSelect" class="form-control month-select mr-2" required>
                    ...
                    <select id="yearSelect" class="form-control year-select mr-2" required>
                        <!-- Year options will be populated dynamically via JavaScript -->
                    </select>
                    <button type="submit" class="btn btn-yes mr-2">Submit</button>
                    <button type="button" class="btn btn-primary d-flex align-items-center" onclick="sendMailToAll()">
                                ...
                                <table id="datatable" class="table data-table table-striped text-center">
                                    ...
                                    <tbody>
                                        ...
                                        $sql = "SELECT 
                                                    ...
                                                    COALESCE(SUM(CASE WHEN l.leave_lop IN ('Lop', 'Paid') THEN l.days_count ELSE 0 END), 0) AS total_leave,
                                                    (SELECT days_count FROM tblleaves WHERE empid = e.id AND `status` = '1' AND leave_lop = '0' AND
                                                    MONTH(STR_TO_DATE(from_date, '%Y-%m-%d')) = '$month'
                                                    ...
                                                    (SELECT COUNT(*) FROM holiday WHERE MONTH(STR_TO_DATE(`start_date`, '%Y-%m-%d')) = '$month'                                      
                                                    AND Company_id = $companyId AND leave_type = 1) AS holiday,
                                                     (SELECT perday_hours FROM schedules WHERE Company_id = $companyId) AS perday_hours,
                                                     (SELECT permonth_days FROM schedules WHERE Company_id = $companyId) AS permonth_days

                                                FROM 
                                                    employee e
                                                LEFT JOIN 
                                                    attendance a ON e.email = a.employee_id AND MONTH(STR_TO_DATE(a.date, '%d-%m-%Y')) = '$month' 
                                                    AND YEAR(STR_TO_DATE(a.date, '%d-%m-%Y')) = '$year' 
                                                LEFT JOIN 
                                                    tblleaves l ON e.id = l.empid AND l.status = '1'                                  
                                                WHERE 
                                                    e.active = 'active' AND e.Company_id = $companyId
                                                GROUP BY 
