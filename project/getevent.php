...
if (isset($_GET['selectedDate'])) {
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            if ($isBirthday) {
                echo '<img class="svg-icon text-secondary mr-3" src="./../uploads/employee/'.$image.'" style="height:70px; width:70px; border-radius:50%;" alt="birthday cake image">';
                ...
                echo '<div class="pl-3 border-left">';
                echo '<h5 class="mb-1">' . $fullName . '</h5>';
                echo '<span style="color: darkblue; font-weight: bold">Congratulations! It is your first day!</span><br> Let us work together with excitement and teamwork to tackle whatever comes our way.';
            }
    ...
    $sql = "SELECT * FROM projects WHERE due_date BETWEEN ? AND ? AND FIND_IN_SET(?, members) 
    AND Company_id = $companyId ORDER BY due_date LIMIT 2";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('sss', $today, $tenDaysLater, $userid);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    $query = "SELECT 
          ...
          LEFT JOIN projects ON projects.id = timeline.project_id
          WHERE timeline.start_t = ? AND timeline.activity = 'meeting' AND (
          FIND_IN_SET(?, timeline.participate_id) > 0) AND timeline.Company_id = ?
          GROUP BY timeline.id, timeline.start_time, timeline.end_time, timeline.project_id,
                   ...
                   timeline.emp_id, timeline.start_t, timeline.participate_id, projects.id, projects.project_name";
$stmt = $conn->prepare($query);
$stmt->bind_param('sii', $today, $userid, $companyId);
$stmt->execute();
$result = $stmt->get_result();
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo 'Assigned by ' . $row['assignedby'] . '<br>';
        echo 'Participants: ' . $row['employee_names']; 
        echo '</div>';
        echo '</div>';
        echo '</div>';
        echo '</div>';
