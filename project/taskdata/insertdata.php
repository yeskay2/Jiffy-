        ...
        if ($_SERVER["REQUEST_METHOD"] === "POST") {
            ...
            if ($stmt) {
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    projects.project_name FROM  tasks t LEFT JOIN employee e_assigned ON t.assigned_to = e_assigned.id LEFT JOIN  
                    employee e_uploader ON t.uploaderid = e_uploader.id LEFT JOIN projects ON t.projectid = projects.id WHERE t.status = 'Todo' AND t.ticket = '$ticket'
    ...
    public function project($uploaderid)
    {
        ...
        if ($_FILES["projectDocument"]["name"]) {
            ...
            if (move_uploaded_file($_FILES["projectDocument"]["tmp_name"], $targetDir . $fileName)) {
                ...
                $query = "INSERT INTO projects (project_name, due_date, priority, description, progress, start_date, members, location, 
    ...
    public function updateProject() {
        ...
        if (!empty($projectDocument['name'])) {
            ...
            if (move_uploaded_file($projectDocument['tmp_name'], $targetDir . $fileName)) {
                $query = "UPDATE projects SET project_name=?, start_date=?, due_date=?, budget=?, priority=?, location=?, description=?, document_attachment=?, department=?, members=?, no_resource=?, no_resources_requried=?, lead_id=?, project_manager_id=?, totalhours=?, perday=?,client_id=?,tasks=? WHERE id=?";
                $stmt = mysqli_prepare($this->conn, $query);
            ...
            $query = "UPDATE projects SET project_name=?, start_date=?, due_date=?, budget=?, priority=?, location=?, description=?, department=?, members=?, modules=?, no_resource=?, no_resources_requried=?, lead_id=?, project_manager_id=?, totalhours=?, perday=?,client_id=?,tasks=? WHERE id=?";
            $stmt = mysqli_prepare($this->conn, $query);
