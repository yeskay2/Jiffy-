...
class UserAuthenticator
{
    ...
    {
         $query = "SELECT * FROM employee WHERE (user_role = 'admin' OR FIND_IN_SET('Admin', Allpannel) > 0 OR FIND_IN_SET('All', Allpannel) > 0) AND email = ?";
...
{  
    ...
    $employeeSql = "SELECT * FROM schedules WHERE Company_id='$companyId'";
    $employeeResult = mysqli_query($this->conn, $employeeSql);
    $row = mysqli_fetch_assoc($employeeResult);

    setcookie("user_id", $userID, time() + (86400 * 30), "/");
    setcookie("Company_id", $companyIds, time() + (86400 * 30), "/");
    if ($row) {
        $companyname = base64_encode($row['Company_name']);
        $companylogo = base64_encode($row['logo']);
        $companyemail = base64_encode($row['emailid']);
        $companyaddress = base64_encode($row['address']);

        setcookie("user_id", $userID, time() + (86400 * 30), "/");
        setcookie("Company_id", $companyIds, time() + (86400 * 30), "/");
        setcookie("companyname", $companyname, time() + (86400 * 30), "/");
        setcookie("companylogo", $companylogo, time() + (86400 * 30), "/");
        setcookie("companyemail", $companyemail, time() + (86400 * 30), "/");
        setcookie("companyaddress", $companyaddress, time() + (86400 * 30), "/");
    } else {
        // $_SESSION['error'] = 'Company details not found.';
