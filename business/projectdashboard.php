...
// $persentage = $projectManager->overview();
?>
<!doctype html>
<html lang="en">
    ...
    <div class="wrapper">
        ...
        <div class="content-page ">           
            ...
            <audio id="incomingCallAudio">
                <source src="./../Employee/script/ring.mp3" type="audio/mpeg">
            ...
            <div class="container-fluid ">
                <div class="row ">
                        ...
                        <div class="card card-block card-stretch card-height">
                            <div class="card-body">
                                <div class="top-block d-flex align-items-center justify-content-between">
                                    <h5>Total Objectives</h5>
                                </div>
                                <?php
                                $query = "SELECT COUNT(*) as total_project FROM objectives WHERE Company_id = $companyId";
                                $result = mysqli_query($conn, $query);
                                if ($result && mysqli_num_rows($result) > 0) {
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        $totalprojects = $row['total_project'];
                                ?>
                                <h3><svg xmlns="http://www.w3.org/2000/svg" height="0.875em" viewBox="0 0 384 512"
                                        fill="#bc2d75" style="margin-right: 10px;">
                                        <path
                                ...
                                <div class="d-flex align-items-center justify-content-between mt-1">
                                    <p class="mb-0">Total Objectives</p>                                    
                                </div>
                                <div class="iq-progress-bar bg-primary-light mt-2">
                                    <span class="bg-primary iq-progress progress-1"
                                        data-percent="<?php echo  $totalprojects; ?>"></span>
                                </div>
                                <?php
                                    }
                                }
                    ...
                    <?php
                        ...
                        $query = "SELECT COUNT(*) as completed_project FROM objectives WHERE progress = 100  AND Company_id = $companyId";
                        $result = mysqli_query($conn, $query);
                        if ($result && mysqli_num_rows($result) > 0) {
                            $row = mysqli_fetch_assoc($result);
                            $completedProjects = $row['completed_project'];
                        } else {
                            $completedProjects = 0;
                        }
                        
                        if ($totalProjects > 0) {
                            $percentageCompleted = ($completedProjects / $totalProjects) * 100;
                        } else {
                            $percentageCompleted = 0;
                        }
                        ?>
                           <div class="col-md-6 col-lg-3" onclick="window.location.href='project.php?status=completed';" style="cursor: pointer;">
                                <div class="card card-block card-stretch card-height">
                                    <div class="card-body">
                                        <div class="top-block d-flex align-items-center justify-content-between">
                                        ...
                                        <h3>
                                            <svg xmlns="http://www.w3.org/2000/svg" height="0.875em" viewBox="0 0 448 512" fill="#007ece"
                                            ...
                                            <span class="counter"><?php echo $completedProjects; ?></span>
                                        </h3>
                                        <div class="d-flex align-items-center justify-content-between mt-1">
                                            <p class="mb-0">Total Completed Objectives</p>
                                            <p class="mb-0"><?php echo round($percentageCompleted, 2); ?>%</p> <!-- Display percentage -->
                                        </div>
                                        <div class="iq-progress-bar bg-info-light mt-2">
                                            <span class="iq-progress progress-1" data-percent="<?php echo $percentageCompleted; ?>"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                               <?php
                                $query = "SELECT COUNT(*) as total_projects FROM tasks WHERE status ='Todo' AND Company_id = $companyId";
                                $result = mysqli_query($conn, $query);
                                if ($result && mysqli_num_rows($result) > 0) {
                                    $row = mysqli_fetch_assoc($result);
                                    $totaltodo = $row['total_projects'];
                                } else {
                                    $totaltodo = 0;
                                }
                                
                                $query = "SELECT COUNT(*) as in_progress_projects FROM tasks WHERE status = 'InProgress' AND Company_id = $companyId ";
                                $result = mysqli_query($conn, $query);
                                
                                if ($result && mysqli_num_rows($result) > 0) {
                                    $row = mysqli_fetch_assoc($result);
                                    $inProgressProjects = $row['in_progress_projects'];
                                } else {
                                    $inProgressProjects = 0;
                                }
                                
                                $totaltask = $totaltodo + $inProgressProjects;
                                
                                if ($totaltask > 0) {
                                    $percentageInProgress = ($inProgressProjects / $totaltask) * 100;
                                } else {
                                    $percentageInProgress = 0;
                                }
                                ?>

                        <div class="col-md-6 col-lg-3" onclick="window.location.href='task.php?taskstatus=Inprogress&&condition=1&&period=All';" style="cursor: pointer;">
                            <div class="card card-block card-stretch card-height">
                                <div class="card-body">
                                    <div class="top-block d-flex align-items-center justify-content-between">
                                        <h5>Inprogress Action Plans</h5>
                                    </div>
                                    <h3>
                                        <svg xmlns="http://www.w3.org/2000/svg" height="0.875em" viewBox="0 0 512 512" fill="#cf9700"
                                            style="margin-right: 10px;">
                                            <path
                                                d="M304 48a48 48 0 1 0 -96 0 48 48 0 1 0 96 0zm0 416a48 48 0 1 0 -96 0 48 48 0 1 0 96 0zM48 304a48 48 0 1 0 0-96 48 48 0 1 0 0 96zm464-48a48 48 0 1 0 -96 0 48 48 0 1 0 96 0zM142.9 437A48 48 0 1 0 75 369.1 48 48 0 1 0 142.9 437zm0-294.2A48 48 0 1 0 75 75a48 48 0 1 0 67.9 67.9zM369.1 437A48 48 0 1 0 437 369.1 48 48 0 1 0 369.1 437z" />
                                        </svg>
                                        <span class="counter"><?php echo $inProgressProjects; ?></span>
                                    </h3>
                                    <div class="d-flex align-items-center justify-content-between mt-1">
                                        <p class="mb-0">Total Inprogress Action plans</p>
                                        <p class="mb-0"><?php echo round($percentageInProgress, 2); ?>%</p> <!-- Display percentage -->
                                    </div>
                                    <div class="iq-progress-bar bg-warning-light mt-2">
                                        <span class="bg-warning iq-progress progress-1" data-percent="<?php echo $percentageInProgress; ?>"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <?php
                        $query = "SELECT COUNT(*) as total_tasks FROM tasks WHERE Company_id = $companyId";
                        $result = mysqli_query($conn, $query);
                        ...
                        $query = "SELECT COUNT(*) as completed_tasks FROM tasks WHERE status = 'Completed' AND Company_id = $companyId";
                        $result = mysqli_query($conn, $query);
                        ...
                        <div class="col-md-6 col-lg-3" onclick="window.location.href='task.php?taskstatus=Completed&&condition=1&&period=All';" style="cursor: pointer;">
                            <div class="card card-block card-stretch card-height">
                                <div class="card-body">
                                    <div class="top-block d-flex align-items-center justify-content-between">
                                        <h5>Completed Action Plans</h5>
                                    </div>
                                    <h3>
                                        <svg xmlns="http://www.w3.org/2000/svg" height="0.875em" viewBox="0 0 512 512" fill="#2f8f81"
                                            style="margin-right: 10px;">
                                            <path
                                                d="M40 48C26.7 48 16 58.7 16 72v48c0 13.3 10.7 24 24 24H88c13.3 0 24-10.7 24-24V72c0-13.3-10.7-24-24-24H40zM192 64c-17.7 0-32 14.3-32 32s14.3 32 32 32H480c17.7 0 32-14.3 32-32s-14.3-32-32-32H192zm0 160c-17.7 0-32 14.3-32 32s14.3 32 32 32H480c17.7 0 32-14.3 32-32s-14.3-32-32-32H192zm0 160c-17.7 0-32 14.3-32 32s14.3 32 32 32H480c17.7 0 32-14.3 32-32s-14.3-32-32-32H192zM16 232v48c0 13.3 10.7 24 24 24H88c13.3 0 24-10.7 24-24V232c0-13.3-10.7-24-24-24H40c-13.3 0-24 10.7-24 24zM40 368c-13.3 0-24 10.7-24 24v48c0 13.3 10.7 24 24 24H88c13.3 0 24-10.7 24-24V392c0-13.3-10.7-24-24-24H40z" />
                                        </svg>
                                        <span class="counter"><?php echo $completedTasks; ?></span>
                                    </h3>
                                    <div class="d-flex align-items-center justify-content-between mt-1">
                                        <p class="mb-0">Total Completed Action Plans</p>
                                        <p class="mb-0"><?php echo round($percentageCompleted, 2); ?>%</p> <!-- Display percentage -->
                                    </div>
                                    <div class="iq-progress-bar bg-success-light mt-2">
                                        <span class="bg-info iq-progress progress-1" data-percent="<?php echo $percentageCompleted; ?>"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
    $(document).ready(function(){
        ...
        function fetchEmployees(selectedDate) {
            ...
            xhr.open('GET', 'projectevent.php?selectedDate=' + selectedDate, true);
            xhr.send();
        }
        ...
        document.addEventListener("DOMContentLoaded", function() {
            var popupTimer; 
            var audio = document.getElementById("incomingCallAudio"); 
            ...
            function fetchDataAndShowPopup() {
                fetch(`call.php?userid=<?php echo $userid ?>`)
                    .then(response => {
                        if (!response.ok) {
                    ...
                    .then(data => {
                        ...
                        popupTimer = setTimeout(closePopup, 60000);
                    })
                    .catch(error => {
            ...
            document.getElementById('confirmButton').addEventListener('click', function() {
                window.location.href =
                    "./../video/index.php?roomID=9475&session_id=<?php echo $userid ?>";
            });
