...
class ProjectManager {
    ...
    public function __construct($conn,$companyId)
    {
        ...
        $query = "";
        if($type_id ==null){
            $query = "SELECT * FROM objectives WHERE  Company_id = $this->companyId AND uploaderid = $userid
             ORDER BY project_name";
        }   
        elseif ($type_id != 1) {
            $query = "SELECT * FROM objectives WHERE REPLACE(CONCAT(',', members, ','), ' ', '') 
            LIKE '%,$userid,%' AND  Company_id = $this->companyId ORDER BY project_name";
        } elseif ($status != null) {
            $query = "SELECT * FROM objectives WHERE progress = 100  AND  Company_id = $this->companyId
             ORDER BY project_name";
        }
        else {
            $query = "SELECT * FROM objectives  WHERE  Company_id = $this->companyId ORDER BY project_name";
        }
...
FROM
    timeline
LEFT JOIN objectives ON timeline.project_id = objectives.id 
